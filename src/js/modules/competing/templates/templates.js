...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('subscription.row.tpl.html',
    '<td>{{ esSubscription.groupName }}</td> \
    <td>{{ esSubscription.averageItemsPerSecond }}</td> \
    <td>{{ esSubscription.knownMessages }}</td> \
    <td>{{ esSubscription.currentMessages }}</td> \
    <td>{{ esSubscription.inFlightMessages }}</td> \
    <td>{{ esSubscription.connectionCount }}</td> \
    <td><div class="status {{ esSubscription.status }}"></div>{{ esSubscription.behindByMessages }} / {{ esSubscription.behindByTime }}</td> \
    <td> \
      <ul class=page-nav> \
        <li ng-show=$root.isAdminOrOps class=page-nav__item> \
        <a ui-sref="^.item.edit({streamId: esSubscription.eventStreamId, groupName: esSubscription.groupName})">Edit</a></li> \
