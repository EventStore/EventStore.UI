...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('subscription.row.header.tpl.html',
    '<td ng-if=esSubscription.show>{{ esSubscription.streamName }} <a class=table-collapsetoggle>&minus;</a></td><td ng-if=!esSubscription.show>{{ esSubscription.streamName }} <a class=table-collapsetoggle>&plus;</a></td><td>{{ esSubscription.averageItemsPerSecond }}</td><td>{{ esSubscription.readyItems }}</td><td>{{ esSubscription.unackedItems }}</td><td>{{ esSubscription.totalItemsProcessed }}</td><td>{{ esSubscription.connectionCount }}</td><td>n/a</td><td>n/a</td>');
}]);
})();
...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('subscription.row.tpl.html',
    '<td>{{ esSubscription.groupName }}</td><td>{{ esSubscription.averageItemsPerSecond }}</td><td>{{ esSubscription.readyItems }}</td><td>{{ esSubscription.unackedItems }}</td><td>{{ esSubscription.totalItemsProcessed }}</td><td>{{ esSubscription.connectionCount }}</td><td>{{ esSubscription.status }}</td><td><a ng-repeat="link in esSubscription.links" ui-sref="^.item.detail({streamId: esSubscription.eventStreamId, groupName: esSubscription.groupName})">{{ link.rel }}</a></td>');
}]);
})();
