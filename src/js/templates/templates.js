...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('projections.item.edit.tpl.html',
    '<header class=page-header><h2 class=page-title>Projection Edit</h2><ul class=page-nav><li class=page-nav__item><button ng-click=start() ng-disabled=isRunning href=#>Start</button></li><li class=page-nav__item><button ng-click=stop() ng-disabled=isStopped>Stop</button></li><li class=page-nav__item><button ng-click=save()>Save</button></li><li class=page-nav__item><button ng-click=reset($event)>Reset</button></li><li class=page-nav__item><a ui-sref=^.details>Back</a></li></ul></header><h3>{{stats.name}} - {{stats.status}}</h3><p><strong>mode:</strong><span>{{stats.mode}}</span></p><div class=container><div class=container-left><h3 class=block-title>Source</h3><form><textarea class=code-full ng-model=query>\n' +
    '				\n' +
    '			</textarea><label><input type=checkbox ng-model=emit>Emit enabled</label></form></div><div class=container-right><h3 class=block-title>Results <span ng-show=stream>(<a ui-sref="streams.item.events({streamId: stream})">{{ stream }}</a>)</span></h3><pre>\n' +
    '{{result | json}}\n' +
    '		</pre><h3 class=block-title>State</h3><pre>\n' +
    '{{state | json}}\n' +
    '		</pre></div></div>');
...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('projections.standard.tpl.html',
    ...
    '				</textarea></td></tr><tr><td>Select Type</td><td><select ng-model=type required class=form-table ng-options="obj.value as obj.name for obj in types"></select></td></tr></tbody></table><ul><li><button type=submit ng-disabled=newProj.$invalid>Create</button></li></ul></form>');
}]);
})();
...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('query.tpl.html',
    '<header class=page-header><h2 class=page-title>Query</h2><ul class=page-nav><li class=page-nav__item><a href=#>Run</a></li><li class=page-nav__item><a href=#>Break</a></li><li class=page-nav__item><a href=#>Debug</a></li></ul></header><div class=container><div class=container-left><h3 class=block-title>Source</h3><form><textarea class=code-full>Text</textarea></form></div><div class=container-right><h3 class=block-title>State</h3><textarea class="code-half code-read-only">Text</textarea><h3 class=block-title>Events</h3><textarea class="code-half code-read-only">Text</textarea></div></div><ul class=page-nav><li class=page-nav__item><a href=#>Update</a></li><li class=page-nav__item><a href=#>Run</a></li></ul><div class=container><div class=container-left><div class=block><h3 class=block-title>Instructions</h3><ol><li>Make sure you use the Google Chrome browser.</li><li>Open a debugger (<kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>J</kbd> on Windows).</li><li>Check the debugger console for errors (occurred while running the projection definition on while loading state and events).</li><li>Click the ‘Run’ button above.</li><li>Watch for errors in the debugger console</li><li>Check the debugging status on XXXXXXXXX.</li><li>Execution will stop on ‘debugger’ statement immediately before invoking your event handler.</li></ol><p>Any log output and emitted events will appear in the log viewer.</p></div></div><div class=container-right><h3 class=block-title>Log Viewer</h3><textarea class="code-half code-read-only code-console">Console</textarea></div></div>');
}]);
})();
...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('streams.item.events.tpl.html',
    ...
    '{{ event.linkMetaData }}					\n' +
    '					</pre></div></td></tr></tbody><tbody ng-hide=streams><tr><td colspan=5><em>No streams for current path: {{ $stateParams | json }}</em></td></tr></tbody></table>');
}]);
})();
